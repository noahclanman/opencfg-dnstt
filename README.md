...
The installer currently uses `apt-get`.
...
```bash
wget https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/opencfg-dnstt.sh
```
...
After installation, open the manager anytime with:
...
DNSTT receives DNS tunnel traffic on UDP `53` and forwards it to Xray on TCP `443`.
...
If your domain uses Cloudflare, the nameserver A record should use:
